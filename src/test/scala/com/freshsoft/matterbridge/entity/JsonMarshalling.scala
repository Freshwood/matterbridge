...
class JsonMarshalling
	...
	"The spray json marshalling" should {
		...
		val slashResponseElement = SlashResponseAttachment("Title", "Title", "URL", "Text", "img_url",
			List(slashResponseField), "#FFFFFF")
		...
		val newsriverResponse = NewsriverResponse("Some Id", "Some Date", "Title", "Text", "url",
			List(newsriverResponseEntity), newsriverRecoverWebsite)
