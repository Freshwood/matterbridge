...
import com.freshsoft.matterbridge.entity.MatterBridgeEntities.{IncomingResponse, RssReaderIncomingModel, RssReaderModel, SlashResponseAttachment}
...
/**
	...
	*/
class RssReaderSenderActor extends Actor with WithActorContext {
	...
	private def buildIncomingResponseFromRssModel(rssReaderModels: List[RssReaderModel]) = {
		...
		def buildMessageAttachments(rssReaderModels: List[RssReaderModel]) = {
			for {
				m <- rssReaderModels
			} yield SlashResponseAttachment(m.title, m.title, m.link, m.description, m.img_url, fields = Nil, author_name = m.author)
		}
