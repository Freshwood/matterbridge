  ...
  private def sendNewsriverResultToIncomingWebhook(
    ...
    Http().singleRequest(
      HttpRequest(uri = newsriverIncomingTokenUrl,
                  ...
                  headers = Nil,
                  entity = HttpEntity(incomingResponse.toJson.toString))) map {
      case HttpResponse(StatusCodes.OK, _, _, _) =>
        log.info(
          s"Successfully send data to $newsriverIncomingTokenUrl with data ${incomingResponse.toJson.toString}")
  ...
  private def buildSlashAndIncomingWebhookResponse(
    ...
    newsriverResponses match {
      ...
      case x: List[NewsriverResponse] if x isEmpty =>
        (Some(SlashResponse("ephemeral", "nothing found", List())), None)
    }
  }

  private def createSlashResponse(
      newsriverResponses: List[NewsriverResponse],
      request: SlashCommandRequest): (SlashResponse, IncomingResponse) = {
    val attachments = for {
    ...
    (SlashResponse(newsriverResponseType, responseText, List()),
     IncomingResponse(responseText, attachments))
  }
